...
Live performance insights for GPU kernels and Python systems code in VS Code, powered by JEV. Pause while coding to see what looks good, what deserves attention, and what to inspect next.

## Install

Requires **VS Code 1.98+**.

1. [Download Kernel Lens (.vsix)](https://github.com/gauravjain14/kernel-lens-jev/releases/download/v0.7.0/kernel-lens-0.7.0.vsix), then choose **Extensions → ⋯ → Install from VSIX**.
2. Run **Kernel Lens: Set AI Gateway API Key** from the Command Palette and enter your [Vercel AI Gateway key](https://vercel.com/docs/ai-gateway/authentication-and-byok).
3. Run **Kernel Lens: Enable Live Insights for This Workspace** in a trusted workspace.

Open a CUDA, Triton or Python file and keep coding. Insights update after a short typing pause. The assumed GPU defaults to **B200**; change it in the sidebar to H100, A100 or Unspecified/custom.

Your key is stored in VS Code secret storage. Enabling a workspace allows focused source context to be sent to AI Gateway. Astra reviews run on demand by default and use the selected model's API pricing.

[Try the MoE demo](examples/multinode-moe-refit/README.md): actual JEV findings on verl's expert-weight export, alongside the upstream memory optimization.

## How it works

Kernel Lens sends your function, relevant helpers, and hardware context to [JEV](https://vercel.com/i/what-is-jev) through Vercel AI Gateway. Kernel Lens defines the performance questions and allowed answers; JEV returns classifications with probabilities.
...
Python lenses cover recognized PyTorch/JAX host reads, compilation boundaries, tensor materialization and distributed collectives. See [framework coverage](docs/SYSTEMS-COVERAGE.md) for supported patterns and limits.
...
[All kernel classification criteria](src/core/systems/kernel-review.ts)
...
`npm run package` builds the extension and creates the VSIX. After upgrading, run **Developer: Reload Window**.
...
[Architecture](docs/SYSTEMS-ARCHITECTURE.md) · [Release notes](docs/RELEASE-0.7.0.md)
